...
export interface DashboardProps {
  initialSecuritySnapshot?: SecuritySnapshot | null;
  loadSecuritySnapshot?: () => Promise<SecuritySnapshot | null>;
}

const vectorLabels: Record<AttackVector, string> = {
  credential_stuffing: 'Credentials',
  ddos: 'DDoS',
  malware: 'Malware',
  phishing: 'Phishing',
...
function SecurityOverview({ snapshot }: { snapshot: SecuritySnapshot }) {
  const trendChart = useMemo(
    () =>
      defineChart({
        marks: [
          areaY(snapshot.trend, {
            id: 'security-area',
            x: (row) => row.label,
            y1: 0,
            y2: (row) => row.attacks,
  ...
  const vectorData = useMemo(
    () =>
      snapshot.vectors.map((item) => ({
        ...item,
        label: vectorLabels[item.vector],
      })),
    [snapshot],
  );
  const vectorChart = useMemo(
    () =>
      defineChart({
        marks: [
          barY(vectorData, {
  ...
  return (
    <section className="security-section" aria-label="Security incident data">
      <div className="security-heading">
        <div>
          <p className="panel-kicker">Topcoat security stream</p>
          <h2>Global incident telemetry</h2>
        </div>
        <span className="security-connected">
          <span className="status-dot" /> Rust API connected
        </span>
      </div>
      <div className="security-metrics">
        <span>
          Active <strong>{snapshot.summary.active_incidents}</strong>
        </span>
        <span>
          Attacks/min <strong>{snapshot.summary.attacks_per_minute}</strong>
        </span>
        <span>
          Blocked{' '}
          <strong>{snapshot.summary.blocked_percent.toFixed(1)}%</strong>
        </span>
        <span>
          Top target <strong>{snapshot.summary.top_target}</strong>
        </span>
      </div>
      <div className="security-chart-grid">
        <article className="panel security-trend-panel">
          <div className="panel-heading">
            <div>
              <p className="panel-kicker">Live attack rate</p>
              <h2>Detections from Topcoat</h2>
            </div>
            <span className="panel-total">Event {snapshot.sequence}</span>
          </div>
          <Chart
            definition={trendChart}
            height={230}
            initialWidth={720}
            ariaLabel="Security attack rate generated by Topcoat"
          />
        </article>
        <article className="panel security-vector-panel">
          <div className="panel-heading">
            <div>
              <p className="panel-kicker">Incident mix</p>
              <h2>Attack vectors</h2>
            </div>
          </div>
          <Chart
            definition={vectorChart}
            height={230}
            initialWidth={420}
            ariaLabel="Security incidents grouped by attack vector"
          />
        </article>
      </div>
    </section>
...
export function Dashboard({
  ...
  useEffect(() => {
    if (!loadSecuritySnapshot) return;

    const refresh = async () => {
      const next = await loadSecuritySnapshot();
      if (next) setSecuritySnapshot(next);
    };
  ...
  return (
    <div className="ops-shell">
      ...
      <main className="ops-main">
        ...
        {securitySnapshot ? (
          <SecurityOverview snapshot={securitySnapshot} />
        ) : (
          <section
            className="security-offline"
            aria-label="Security service status"
          >
            <div>
              <p className="panel-kicker">Topcoat security stream</p>
              <h2>Security telemetry awaiting Rust service</h2>
            </div>
            <code>pnpm exec nx run topcoat-security:run</code>
          </section>
        )}
